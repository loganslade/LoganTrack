...
back_track <- function(df, positions, i_jump, m_jump, s_jump, smod, m_thresh, frame_limit){
...
for(p in positions){
  ...
  for(a in 1:1){                  
    ...
    for(b in 1:nobj_frame){
      ...
      #print(paste0("Tracking object ",b, " of ",nobj_frame, " in position ",p))
      if(length(tracking) > 0 && cur_object$i_id %in% tracking$i_id[tracking$position == p & tracking$time == cur_frame]) {next}
        ...
        for(c in (cur_frame-1):frame_limit){ 
          if(c == nframe-1){
            ...
            next_object <- filter(search, between(search$x,(cur_object$x - max.jump), (cur_object$x + max.jump)) &
                                    between(search$y,(cur_object$y - max.jump), (cur_object$y + max.jump))) 
            if(nrow(next_object) == 1){
              ...
              next_object <- next_object %>% mutate(distance = sqrt(((cur_object$x-x)^2)+((cur_object$y-y)^2)))
            ...
            x_cur <- cur_object$x
            y_cur <- cur_object$y 
            cur_object$enext_x <- x_cur + x_traj
            cur_object$enext_y <- y_cur + y_traj
            search <- track_set[track_set$time == c,]
            next_object <- filter(search, between(search$x,((cur_object$x+(x_traj*smod))- max.jump), ((cur_object$x+(x_traj*smod)) + max.jump)) &
                                    between(search$y,((cur_object$y+(y_traj*smod)) - max.jump), ((cur_object$y+(y_traj*smod)) + max.jump))) 
...
forward_track <- function(df, positions, i_jump, m_jump, s_jump, smod, m_thresh, frame_limit_f){
  df$time <- as.numeric(df$time)
  tracking <- list()
  cur_frame <- 1
  nframe <- max(df$time)
...
back_track_dist <- function(df, positions, i_jump, m_jump, s_jump, smod, m_thresh){
  df$time <- as.numeric(df$time)
  numcol <- c("area", "axis_major_length", "eccentricity", "cdkact")
  tracking <- list()
  cur_frame <- max(df$time)
  nframe <- max(df$time)
  x <- 0
  ...
  for(p in positions){
    old <- Sys.time()
    sample <- df %>% filter(n_position == p)
    track_set <- sample %>% dplyr::select(c("x","y", "i_id", "image", "time", "scale_ms", "area", "axis_major_length",
                                            "eccentricity", "cdkact"))
    print(p)
    for(a in 1:1){                  
      nobj_frame <- sample %>% filter(time == cur_frame) %>% nrow(.)
      ...
      for(b in 1:nobj_frame){
        ...
        track_id <- cur_object$i_id
        #print(paste0("Tracking object ",b, " of ",nobj_frame, " in position ",p))
        if(length(tracking) > 0 && cur_object$i_id %in% tracking$i_id[tracking$position == p]) {next}
        else{
          for(c in (cur_frame-1):0){ 
            if(c == nframe-1){
              max.jump <- i_jump
              x_cur <- cur_object$x
              y_cur <- cur_object$y 
              ...
              next_object <- filter(search, between(search$x,(cur_object$x - max.jump), (cur_object$x + max.jump)) &
                                      between(search$y,(cur_object$y - max.jump), (cur_object$y + max.jump))) 
              if(nrow(next_object) == 1){
                if(length(tracking) > 0 && next_object$i_id %in% tracking$i_id[tracking$position == p]) {break}
                ...
                tracking$enext_x[x] <- next_object$x+x_traj
                tracking$enext_y[x] <- next_object$y+y_traj
                tracking$dist[x] <- euc_dist(dplyr::select(next_object, all_of(numcol)), dplyr::select(cur_object, all_of(numcol)))
                cur_object <- next_object}
              else if(nrow(next_object) >1){
                next_object <- next_object %>% mutate(distance = sqrt(((cur_object$x-x)^2)+((cur_object$y-y)^2)))
                d_min <- min(next_object$distance)
...
track_link <- function(positions, btracked, ftracked, jump, m_thresh, StabilityValue){
...
back_track_jitter <- function(df, positions, i_jump, m_jump, s_jump, 
                              post_jitter_frame, smod, m_thresh, frame_limit){
  df$time <- as.numeric(df$time)
  tracking <- list()
  cur_frame <- max(df$time)
  nframe <- max(df$time)
  ...
  for(p in positions){
    old <- Sys.time()
    sample <- df %>% filter(n_position == p)
    jitter_x <- sample$jitter_x[sample$time == post_jitter_frame][1]
    jitter_y <- sample$jitter_y[sample$time == post_jitter_frame][1]
    track_set <- sample %>% dplyr::select(c("x","y", "i_id", "n_image", "time", "scale_ms"))
      ...
      for(b in 1:nobj_frame){
        ...
        if(length(tracking) > 0 && cur_object$i_id %in% tracking$i_id[tracking$position == p & tracking$time == cur_frame]) {next}
          ...
          for(c in (cur_frame-1):frame_limit){
            ...
            if(c == post_jitter_frame-1){shift_x <- jitter_x
            shift_y <- jitter_y}
            else{shift_x <- 0 
            shift_y <- 0}
...
forward_track_jitter <- function(df, positions, i_jump, m_jump, s_jump, 
                                 post_jitter_frame, smod, m_thresh, frame_limit_f){
  df$time <- as.numeric(df$time)
  tracking <- list()
  cur_frame <- 1
  nframe <- max(df$time)
      ...
      for(b in 1:nobj_frame){
        ...
        if(length(tracking) > 0 && cur_object$i_id %in% tracking$i_id[tracking$position == p & tracking$time == cur_frame]) {next}
          ...
          for(c in (cur_frame+1):frame_limit_f){ 
            ...
            if(c == post_jitter_frame){shift_x <- jitter_x
            shift_y <- jitter_y}
